...
\red{\textbf{Notes}
    \begin{itemize}
        \item collect estimates of $\tau$ and $\theta$ into tables.
        \item Move all descriptions of MCMC setting etc. into SI.  Provide data files in SI.
    \end{itemize}
}
...
\textbf{Data.}  Nine giraffe subspecies have been recoginised: \textit{camelopardalis},
\textit{angolensis}, \textit{antiquorum}, \textit{giraffa}, \textit{peralta},
\textit{reticulata}, \textit{rothschildi}, \textit{thornicrofti} and
\textit{tippelskirchi}.  \red{Genetically isolated populations exhibit phenotypic
    convergence, which complicates the recognition of distinct species \citep{Fiser2018}.} 
Based on morphological and molecular data, several hypotheses of delimitation have been
presented, ranging from one species to as many as six.
...
supported five species, which was the maximum that was tested. \red{[How are structure
   and stacey used to delimit species?  Phylogenetic programs MrBayes, PhyML, and SuperTRI
   are not for delimiting species?  Why are the \textsc{bpp} results different in the two
   analyses?]} \\ %
\red{[Here, the original authors tested if bpp supports 2, 3, 4, or 5 species. They
   found that 5 were supported.  I retested the dataset (which contained 9 populations)
   without specifying the possible number of species (in A11 mode) and found that BPP
   supported 6 species. The original authors never tested a 6 species hypothesis.]}

We analyzed the data using our pipeline. We augmented the data with further (?) sequences
from the outgroup species, \textit{Okapia johnstoni}.  \red{[The use of outgroup is
    unnecessary and complicates comparison with previous results.  Shall we remove the
    outgroup and use the same data as \citet{Petzold2020}.]}  We used the guide tree for the
nine subspecies of \citet{Petzold2020}.  The mutation rate was set at $\mu = 2.12 \times
10^{-8}$ per site per generation \citep{Chen2019}.  \textsc{bpp} supported six species.

Hierarchical merge and split analyses were conducted with the default \textit{gdi}
thresholds of 0.2 and 0.7, \red{and default generation thresholds of 1000 and 10000 for
   each method respectively.} The split algorithm suggested three species while the merge
algorithm suggested six (fig.~\ref{fig:giraffe}).  Both grouped the Eastern populations
...
merge algorithm recognized four species.   Mitochondrial introgression has been detected
between the \textit{rotschildi} and \textit{reticulata} subspecies \citep{Fennessy2016}.

\textbf{2023.3.29-giraffe.}  Based on the observations of mitochondrial haplotypes and
...
the prior \red{G$(1,100)$} with mean 0.01 migrant individuals per generation.  Merge and
split analyses were conducted with the animal specific $gdi$ thresholds of 0.3 and 0.7, as
recommended by \citet{Jackson2017}.  Both algorithms suggested three species, separating
the northern, eastern, and southern populations into distinct species.  The result is
consistent with \citet{Petzold2020}.


\subsection{Species delimitation of \textit{Albinaria} snails in Crete}
...
\textbf{Data.}  The genus \textit{Albinaria} is one of the largest radiations of land
snails, with up to 140 species found in their native habitats of Greece and south-western
Turkey (Schilthuizen 2018).  Delimiting species in such ongoing radiations is challenging
due to the small differences between the incipient species and possible gene flow between
lineages.  \citet{Bamberger2022} applied three molecular delimitation methods to data of
393 nuclear loci (with an average length of 209bp), sampled from 93 individuals across the
48 populations in the \textit{Albinaria cretensis} complex from Western Crete.  \red{While
    traditional morphological delimitation \ref{} supported 3-9 species, Gaussian clustering
    \ref{?} supported 6 species, \textsc{admixture} \citep{Alexander2011} supported 11-17
    species}, while \textsc{bpp} supported 45-48 species.

We used the phylogenetic tree for the 48 populations produced by \citet{Bamberger2022} as
the guide tree.  \red{[delete?  As no nuclear mutation rate was available for snails, we
    estimated it by taking a known mtDNA $\mu$ derived in the snail \textit{Radix balthica}
    ($\mu = 7.95 \times 10^{-7}$) \citep{Bolotov2017}, and multiplying it by the mitochondrial
    to nuclear gene mutation rate ratio (0.24) recently estimated for gastropods
    \citep{Duda2021}, which yielded $\mu = 1.92 \times 10^{-7}$.] }

We used the merge and split algorithms, with the default \textit{gdi} thresholds of 0.2
and 0.7, and default generation thresholds of 1000 and 10000. [??]

\textbf{Output and interpretation.}  The progressive split and merge algorithms suggested
29 and 44 species, respectively (fig.~\ref{fig:snail}).  \red{[delete?] The large range
    may be due to the fact that some speciation processes are ongoing, so that the species
    status may be ambiguous \citep{Stankowski2021}.}
